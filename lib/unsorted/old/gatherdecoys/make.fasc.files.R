...
pdbdir <- as.character(lapply(strsplit(gsub('//','/',fasc$filename),'/'),function(x) x[length(x)-1]))
for( dir in unique(pdbdir) ) {
  if(!file.exists(paste("rand_",dir,sep='')))
    dir.create(paste("rand_",dir,sep=''))
  s <- fasc[pdbdir==dir,]
  N <- min(200,dim(s)[1])
  s <- s[ order(runif(dim(s)[1]))[1:N] ,]
  write.data.frame(s,file=paste("rand_",dir,'.fasc',sep=''))
}
