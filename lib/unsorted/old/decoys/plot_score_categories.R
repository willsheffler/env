...
plotcategories <- function(decoydir,dataset,prot,xlim=NULL,ylim=NULL,pch='+') {
  ddir <- paste(decoydir,dataset,'/',prot,sep='')
  allfile  <- paste(ddir,'/all.fasc',sep='')
  gsbrfile <- paste(ddir,'/gsbr.fasc',sep='')
  gsgrfile <- paste(ddir,'/gsgr.fasc',sep='')
  natfile  <- paste(ddir,'/relax_native.fasc',sep='')
  nat  <- NULL
  ...
  if(is.null(ylim)) {
    ...
    if(!is.null(nat))
      ylim <- c( min(ylim[1], min(nat$score)) , max( ylim[2], quantile(nat$score ,0.9) ) )
  }
  par(cex=0.5)
  plot(all$rms,  all$score, xlim=xlim, ylim=ylim, pch='.',
       main=paste(dataset,'\n',prot,'nsamp:',dim(all)[1]))
  
  r <- grep('fasc',dir(ddir,'rand_'),value=T)
  print(paste(prot,r))
  col = rainbow(length(r))
  if(length(r)) {
    for(ii in 1:length(r)) {
      rfasc <- read.table(ddir,paste(r[ii],sep=''),header=T,comment.char='')
      points(rfasc$rms,rfasc$score,col=col[ii],pch='.')
    }
  }
...
if(0)
  plotcategories('~/data/decoys/','phil_homolog','1af7')
...
plotrmscategories <- function(decoydir,dataset,prot,xlim=NULL,ylim=NULL,pch='+') {
  ...
  if(file.exists(allfile)) {
    ...
    for(i in 1:10) {
      file <- paste(ddir,'/rms',i-1,'.fasc',sep='')
      if(file.exists(file)) {
...
makeplots <- function() {
  ...
  figdir   <- "/users/sheffler/data/decoys/fig"
  ...
  gsbrgsgrplot(decoydir,'phil_homolog',prots,figdir,xlim,ylim)
  rmsplot     (decoydir,'phil_homolog',prots,figdir,xlim,ylim)


  xlim  <- c(0,10)
  ylim  <- c(-200,-100)
  gsbrgsgrplot(decoydir,'kira_fullatombenchmark_test',c('1aa3'),figdir,xlim,ylim)
