...
if ($#ARGV < 2) {
    print STDERR "usage: $0 <ssafile> <startres[,startres2...]> <stopres[,stopres2...]>\n";
    exit -1;
}
$ssafile      = shift @ARGV;
...
# read ssa_in
@ssa_in = ();
foreach $line (&fileBufArray ($ssafile)) {
    next if ($line =~ /^\s*\>/);
    $line =~ s/\s+//g;
    push (@ssa_in, split (//, $line));
}
...
# build ssa out
@ssa_out = ();
for ($res_i=0; $res_i <= $#ssa_in; ++$res_i) {
    next if (! $mask[$res_i]);
    push (@ssa_out, $ssa_in[$res_i]);
}
