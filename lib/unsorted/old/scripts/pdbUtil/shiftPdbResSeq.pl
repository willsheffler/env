...
for ($i=0, $atom_i=0; $i <= $#buf; ++$i) {
	...
	$res_num = substr ($buf[$i], 22, 4);   # doesn't include insertion code A, B, etc.

	$res_shift = $res1 - $res_num  if (! defined $res_shift);

	if (! defined $lastresnum{$chain} || $res_num ne $lastresnum{$chain}) {
	    $lastresnum{$chain} = $res_num;
	}
	substr ($buf[$i], 22, 4) = sprintf ("%4d", $res_num + $res_shift);
    }
